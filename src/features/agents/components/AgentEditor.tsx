  ...
  return (
    <div className="agent-editor-fieldset">
          ...
          <div className="field-group">
            ...
            >
              {visibilityScopeOptions.map((option) => {
                const checked = form.visibilityScopes.includes(option.value);
                const presentation = visibilityPresentation(
                  option.value,
                  option.label,
                  t,
                );
      ...
      <AgentFormSection
        ...
        icon="tune"
        title={t("agents.interaction.title")}
      >
        <p>{t("agents.interaction.description")}</p>
        <div
          className="agent-choice-grid agent-interaction-choice-grid"
          role="group"
          aria-labelledby={`${AGENT_FORM_SECTION_IDS[5]}-title`}
        >
          {interactionFields.map((key) => {
            const attachment = key === "localFiles" || key === "chatRecords";
            const checked = attachment
              ? (form.interactionConfig?.attachment?.[key] ??
                interaction.attachment[key])
              : (form.interactionConfig?.[key] ?? interaction[key]);
            const presentation = interactionPresentation(key, t);
            return (
              <label
                key={key}
                className={`agent-choice-card ${checked ? "is-selected" : ""}`}
              >
                ...
                <Switch
                  ...
                  onChange={(value) => {
                    updateForm({
                      interactionConfig: attachment
                        ? {
                            ...form.interactionConfig,
                            attachment: {
                              ...form.interactionConfig?.attachment,
                              [key]: value,
                            },
                          }
        ...
        <UiButton
          className="agent-interaction-reset"
          variant="ghost"
          size="sm"
          onClick={() => updateForm({ interactionConfig: undefined })}
        >
          {t("agents.interaction.reset")}
        </UiButton>
      </AgentFormSection>
      <AgentFormSection
      ...
      >
        <AgentCapabilitiesEditor
          ...
          getToolSourceLabel={(tool) =>
            toolSourceLabel(tool.sourceCategory, t) || tool.kind
          }
      ...
      >
        <div className={AGENT_FORM_GRID_CLASS_NAME}>
          <div className={AGENT_FORM_FULL_WIDTH_CLASS_NAME}>
            <div className="agent-prompt-field-heading">
              ...
              <Tooltip title={t("agentConsole.prompt.soul.description")}>
                <button
                  type="button"
                  className="agent-prompt-help"
                  aria-label={t("agentConsole.prompt.soul.description")}
                >
            ...
            <div className="agent-prompt-field-heading">
              ...
              <Tooltip title={t("agentConsole.prompt.agents.description")}>
                <button
                  type="button"
                  className="agent-prompt-help"
                  aria-label={t("agentConsole.prompt.agents.description")}
                >
                  <MaterialIcon name="info" />
                </button>
              </Tooltip>
          ...
          <div className={AGENT_FORM_FULL_WIDTH_CLASS_NAME}>
            <div className="agent-prompt-field-heading">
              <span
                id="agent-greetings-label"
                className="agent-prompt-field-label"
              >
                {t("agentConsole.field.greetings")}
              </span>
              <Tooltip
                title={t("agentConsole.prompt.greetings.description", {
                  agentToken: "${agent}",
                })}
              >
                <button
                  type="button"
                  className="agent-prompt-help"
                  aria-label={t("agentConsole.prompt.greetings.description", {
                    agentToken: "${agent}",
                  })}
                >
                  <MaterialIcon name="info" />
              ...
              <UiButton
                className="agent-prompt-heading-action"
                size="sm"
                variant="ghost"
                onClick={() =>
                  updateForm({
                    greetingsText: promptEntriesToJson([
                      ...greetingEntries,
                      "",
                    ]),
                  })
              ...
              >
                ...
                {t("agentConsole.prompt.addGreeting")}
              </UiButton>
            </div>
            <div
              className="agent-prompt-entry-list"
              role="group"
              aria-labelledby="agent-greetings-label"
            >
              {(greetingEntries.length ? greetingEntries : [""]).map(
                (entry, index) => (
                  <div className="agent-prompt-entry" key={`greeting-${index}`}>
                    <Input
                      id={index === 0 ? "agent-greetings-input" : undefined}
                      aria-label={t("agentConsole.prompt.greetings.item", {
                        index: index + 1,
                      })}
                      placeholder={t(
                      ...
                      onChange={(event) => {
                        const next = [
                          ...(greetingEntries.length ? greetingEntries : [""]),
                        ];
                        next[index] = event.target.value;
                        updateForm({
                          greetingsText: promptEntriesToJson(next),
                        });
                      }}
                    />
                    <UiButton
                      size="mini"
                      variant="ghost"
                      aria-label={t("agentConsole.prompt.removeItem", {
                        index: index + 1,
                      })}
                      onClick={() =>
                        updateForm({
                          greetingsText: promptEntriesToJson(
                            greetingEntries.filter(
          ...
          <div className={AGENT_FORM_FULL_WIDTH_CLASS_NAME}>
            <div className="agent-prompt-field-heading">
              <span
                id="agent-introductions-label"
                className="agent-prompt-field-label"
              >
                {t("agentConsole.field.introductions")}
              </span>
              <Tooltip
                title={t("agentConsole.prompt.introductions.description")}
              >
                <button
                  type="button"
                  className="agent-prompt-help"
              ...
              <UiButton
                ...
                onClick={() =>
                  updateForm({
                    introductionsText: promptEntriesToJson([
                      ...introductionEntries,
                      "",
                    ]),
                  })
              ...
              >
                <MaterialIcon name="add" />
                {t("agentConsole.prompt.addIntroduction")}
              </UiButton>
            </div>
            <div
              className="agent-prompt-entry-list"
              role="group"
              aria-labelledby="agent-introductions-label"
            >
              {(introductionEntries.length ? introductionEntries : [""]).map(
                (entry, index) => (
                  <div
                    className="agent-prompt-entry"
                    key={`introduction-${index}`}
                  >
                    <Input
                      id={index === 0 ? "agent-introductions-input" : undefined}
                      aria-label={t("agentConsole.prompt.introductions.item", {
                        index: index + 1,
                      })}
                      placeholder={t(
                      ...
                      onChange={(event) => {
                        const next = [
                          ...(introductionEntries.length
                            ? introductionEntries
                            : [""]),
                        ];
                        next[index] = event.target.value;
                        updateForm({
                          introductionsText: promptEntriesToJson(next),
                        });
                      }}
                    />
                    <UiButton
            ...
            <div className="agent-prompt-field-heading">
              ...
              <Tooltip title={t("agentConsole.prompt.wonders.description")}>
                <button
                  type="button"
                  className="agent-prompt-help"
                  aria-label={t("agentConsole.prompt.wonders.description")}
                >
                  <MaterialIcon name="info" />
                </button>
              </Tooltip>
              <UiButton
                ...
                onClick={() =>
                  updateForm({
                    wondersText: promptEntriesToJson([...wonderEntries, ""]),
                  })
                }
              >
                <MaterialIcon name="add" />
                {t("agentConsole.prompt.addWonder")}
              </UiButton>
            </div>
            <div
              className="agent-prompt-entry-list"
              role="group"
              aria-labelledby="agent-wonders-label"
            >
              {(wonderEntries.length ? wonderEntries : [""]).map(
                (entry, index) => (
                  <div className="agent-prompt-entry" key={`wonder-${index}`}>
                    <Input
                      id={index === 0 ? "agent-wonders-input" : undefined}
                      aria-label={t("agentConsole.prompt.wonders.item", {
                        index: index + 1,
                      })}
                      ...
                      onChange={(event) => {
                        const next = [
                          ...(wonderEntries.length ? wonderEntries : [""]),
                        ];
                        next[index] = event.target.value;
                    ...
                    <UiButton
                      ...
                      onClick={() =>
                        updateForm({
                          wondersText: promptEntriesToJson(
                            wonderEntries.filter(
                              (_, entryIndex) => entryIndex !== index,
                            ),
                          ),
                        })
