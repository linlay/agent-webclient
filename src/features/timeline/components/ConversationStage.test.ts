...
describe("ConversationStage", () => {
  ...
  it.each([
    ["${agent}与你一起工作", "小宅与你一起工作"],
    ["今天想和${agent}一起做点什么？", "今天想和小宅一起做点什么？"],
    ["${agent}和${agent}", "小宅和小宅"],
    ["未知 ${other} <script>text</script>", "未知 ${other} &lt;script&gt;text&lt;/script&gt;"],
  ])("renders greeting tokens as text without alternative agents: %s", (template, expected) => {
    mockGreeting = template;
    const state = createInitialState();
    const currentRow = createAgentWorkerRow();
    mockCurrentWorker = {
      key: currentRow.key,
