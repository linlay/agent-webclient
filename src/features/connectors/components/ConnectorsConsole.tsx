  ...
  return <div className={`management-page-console ${styles.console}`}>
    ...
    <div className={styles.body}>
      <aside className={styles.list} aria-label={t("connectors.list.label")}>
        <div className={styles.toolbar}><h2>{t("settingsMenu.connectors")}</h2><UiButton size="sm" variant="ghost" iconOnly aria-label={t("connectors.action.refresh")} disabled={runtime.loading || runtime.saving} onClick={() => void runtime.refreshCatalog()}><MaterialIcon name="refresh" /></UiButton></div>
        ...
        <div className={styles.listScroll}>
          <Spin spinning={runtime.loading}>
            {items.map(item => <button type="button" className={styles.listItem} aria-current={!showUnassigned && selected?.id === item.id ? "true" : undefined} disabled={runtime.saving} key={item.id} onClick={() => { setShowUnassigned(false); runtime.selectConnector(item.id); }}>
        ...
        {showUnassigned ? <><h2>{t("connectors.tools.unassigned", { count: unassigned.length })}</h2><p className={styles.hint}>{t("connectors.tools.unassignedHint")}</p><ConnectorTools tools={unassigned} /></> : !selected ? <p className={styles.empty}>{routeId && !runtime.loading ? t("connectors.detail.notFound", { id: routeId }) : t("connectors.detail.empty")}</p> : <>
          ...
          {view === "overview" ? <ConnectorOverview item={selected} tools={tools} /> : <>
            <div className={styles.files} aria-label={t("connectors.field.file")}>
              {connectorFiles(selected).map(file => <button type="button" key={file} disabled={runtime.saving} aria-pressed={runtime.file === file} onClick={() => runtime.selectFile(file)}>{file}</button>)}
            </div>
            ...
            <Spin spinning={runtime.detailLoading}>
              {runtime.detail && runtime.detail.id === selected.id && runtime.detail.file === runtime.file && <ConnectorConfigEditor key={`${selected.id}/${runtime.file}`} file={runtime.file} draft={runtime.draft} disabled={runtime.saving} onChange={runtime.updateDraft} />}
            </Spin>
            <footer className={styles.actions}>
              <UiButton variant="primary" size="sm" loading={runtime.saving} disabled={!runtime.dirty || runtime.detailLoading || !runtime.detail?.sha256} onClick={() => void runtime.save()}>{t("connectors.action.save")}</UiButton>
              <UiButton variant="ghost" size="sm" disabled={runtime.saving || runtime.detailLoading} onClick={runtime.reload}>{t("connectors.action.reload")}</UiButton>
