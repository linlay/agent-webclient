...
} from "@/features/connectors/lib/connectorDefinition";
...
export function ConnectorConfigEditor({
  ...
  try {
    parsed = parseConnectorDefinition(draft);
  } catch {
    /* Invalid drafts remain editable as source. */
  }
  const manifest = file === "connector.json";
  const sourceMode =
    source || !parsed || file === "cli.json" || file === "view.json";
  const readOnlyField = (label: string, value: unknown) => (
  ...
  const metadata = (
    <div className={styles.manifestMetadata}>
      ...
      {readOnlyField(
        t("connectors.field.primaryType"),
    ...
    <label className={styles.field} key={path.join(".")}>
      ...
      {multiline ? (
        <Input.TextArea
          ...
          onChange={(event) =>
            onChange(updateConnectorField(draft, path, event.target.value))
          }
        />
      ) : (
        <Input
    ...
    <div className={styles.config}>
        ...
        {file !== "cli.json" && file !== "view.json" && (
          ...
          >
            {t(
              sourceMode
                ? manifest && readOnly
                  ? "connectors.action.information"
                  : "connectors.action.form"
                : "connectors.action.source",
            )}
          </UiButton>
        )}
      </div>
      ...
      {sourceMode ? (
        <div
          ...
          role="group"
          aria-label={t("connectors.field.json")}
        >
          <CodeEditor
        ...
        <div className={styles.manifest}>
          ...
          {field(
            t("connectors.field.description"),
            ["description"],
            parsed?.description,
            true,
          )}
            ...
            return (
              <section className={styles.group} key={key}>
                <div className={styles.toolbar}>
                  <strong>{key}</strong>
                  <code>{String(server.type || "")}</code>
                </div>
                <div className={styles.formGrid}>
                  {field(
                    t(
                      stdio
                        ? "connectors.field.command"
                        : "connectors.field.url",
                    ),
                    ["mcpServers", key, stdio ? "command" : "url"],
                  ...
                  <label className={styles.field}>
                    ...
                    <Input
                      ...
                      readOnly={readOnly}
                      value={
                        typeof server.timeout === "number" ? server.timeout : ""
                      }
                      onChange={(event) => {
