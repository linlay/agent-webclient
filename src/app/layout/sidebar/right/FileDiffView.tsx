...
import {
  buildLineDiffRows,
  type FileDiffRow,
} from "@/app/layout/sidebar/right/fileDiff";
...
const FILE_DIFF_CLASS_NAME =
  "right-sidebar-file-diff tw:font-code tw:text-[11px] tw:leading-[1.45] tw:w-fit tw:min-w-full";
...
const FILE_DIFF_LINE_TONE_CLASS_NAMES: Record<FileDiffRow["type"], string> = {
  ...
  delete:
    "tw:bg-[color-mix(in_srgb,var(--danger)_10%,transparent)] tw:[&_.right-sidebar-file-diff-marker]:text-danger tw:border-l-2 tw:border-[color-mix(in_srgb,var(--danger)_70%,transparent)]",
  context: "",
};
...
export const FileDiffView: React.FC<{
  ...
  const firstChangeIndex = React.useMemo(
    () => rows.findIndex((row) => row.type !== "context"),
    [rows],
  );
...
const FileDiffLine = React.forwardRef<HTMLDivElement, { row: FileDiffRow }>(
  ({ row }, ref) => {
    ...
    return (
      <div className={className} role="row" ref={ref}>
