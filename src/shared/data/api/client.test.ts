...
describe('data client query payloads', () => {
  ...
  it('downloads the complete HTML document generated by Agent Platform', async () => {
    const html = '<!doctype html><title>Conversation</title>';
        ...
        get: (key: string) => {
          switch (key.toLowerCase()) {
            ...
            case 'content-disposition':
              return 'attachment; filename="conversation.html"';
            default:
              return null;
          }
        },
    ...
    expect(fetchMock.mock.calls[0][0]).toBe('/api/chat/export?chatId=chat_1&format=html');
    expect((fetchMock.mock.calls[0][1]?.headers as Record<string, string>).Accept).toBe('text/html');
    expect((fetchMock.mock.calls[0][1]?.headers as Record<string, string>)['X-Conversation-Export-Asset-Origin']).toBe('http://127.0.0.1:11961');
  ...
  it('rejects HTML export early when Content-Length exceeds 20 MiB', async () => {
    const blob = jest.fn();
    fetchMock.mockResolvedValueOnce({
      ok: true,
      status: 200,
  ...
  it('rejects an oversized HTML Blob when Content-Length is missing', async () => {
    fetchMock.mockResolvedValueOnce({
      ok: true,
      status: 200,
      headers: {
      ...
      blob: async () =>
        new Blob([Buffer.alloc(MAX_CONVERSATION_HTML_BYTES + 1)], {
