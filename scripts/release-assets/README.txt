...
3. Edit `.env` so `BASE_URL`, optional `WS_BASE_URL`, and `VOICE_BASE_URL` point to the backend endpoints reachable from the deployment target.
...
- `WS_BASE_URL` is optional and points to the main WebSocket upstream. It defaults to `BASE_URL`.
