...
3. Edit `.env` so `BASE_URL`, `CONVERSATION_EXPORT_ASSET_ORIGIN`, and optional `VOICE_BASE_URL` match the deployment target.
...
- `CONVERSATION_EXPORT_ASSET_ORIGIN` is required for HTML export and points to the Tunnel origin serving `/assets/conversation-export/`.
