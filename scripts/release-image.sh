...
ensure_host_dependencies() {
  if [[ -x "$REPO_ROOT/node_modules/.bin/webpack" ]]; then
    echo "[release-image] reusing existing host dependencies"
    return 0
  fi

  echo "[release-image] installing dependencies on host..."
  (
    cd "$REPO_ROOT"
    if [[ -f package-lock.json ]]; then
      npm ci
      if [[ ! -x "$REPO_ROOT/node_modules/.bin/webpack" ]]; then
        echo "[release-image] npm ci did not produce a usable webpack binary; retrying with npm install"
        npm install
      fi
    else
      npm install
...
BUNDLE_NAME="agent-webclient-image-${VERSION}-linux-${ARCH}"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/agent-webclient-release-image.XXXXXX")"
...
if [[ ! -f "$BUILD_ENV_FILE" ]]; then
  ...
  echo "[release-image] root .env not found; using .env.example defaults for production build"
fi

ensure_host_dependencies

echo "[release-image] building frontend on host..."
(
  cd "$REPO_ROOT"
...
tar -czf "$BUNDLE_TAR_TMP" -C "$TMP_DIR" agent-webclient

rm -rf "$REPO_ROOT/dist"
mkdir -p "$(dirname "$BUNDLE_TAR")"
mv "$BUNDLE_TAR_TMP" "$BUNDLE_TAR"

echo "[release-image] done: $BUNDLE_TAR"
