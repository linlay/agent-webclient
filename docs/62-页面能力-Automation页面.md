...
- **属性**：名称、Cron 与快捷选择、智能体、时区、剩余次数、启用状态、描述。
- **执行**：最近执行记录与刷新入口；新建状态展示保存后可查看记录的空状态。

吸顶锚点栏使用短词导航（中文「属性 / 执行」，英文 "Properties / Executions"），右侧提供源码切换、启停与「图标 + 保存」按钮（创建态按钮提示为“创建自动化”，保存按钮与相邻图标按钮等高）。智能体下拉的每个选项展示 Agent 图标、名称与角色（角色为小字淡色），选中后显示图标与名称；Cron 快捷选择是点击即赋值的动作菜单——点击某个预设直接把 cron 写入表单，无选中态、不渲染为下拉选项。
