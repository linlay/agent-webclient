...
- 检查中：composer 显示“正在检查 Agent 状态…”，暂不开放执行。
...
不可用期间不显示输入、附件、发送和审批控件，发送/重发、steer 与 awaiting 提交函数也检查同一执行状态；已有 Run 的停止入口保留。历史阅读、复制和搜索继续可用。此状态是 UI 准入提示，服务端仍在每次 query/续聊时检查真实配置和权限。Team 仍使用其原有执行路径。
