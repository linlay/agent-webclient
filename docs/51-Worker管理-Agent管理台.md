...
已保存的目录型 Agent 可在“技能”行导入 ZIP 形式的专属 Skill。页面不要求手填 Key，后端从 ZIP 的 `SKILL.md` frontmatter 读取 `key`（没有则 `name`）。导入自动启用，文件只属于当前 Agent；下拉选项明确标注“技能中心”“专属”或“专属 · 覆盖技能中心”。同名技能中心 Skill 会在第一次导入校验后要求确认，确认后才会由当前 Agent 覆盖。请求只发送新版 `confirmCenterOverride` 字段，并只解析 `overridesCenter`。Select 标签的移除只停止启用，文件仍可再次启用；真实删除必须点击专属 Skill 行的危险操作并二次确认。新建 Agent、非目录 Agent、有未保存修改或正在执行保存/删除时，导入入口禁用。页面不提供专属 Skill 的文件树编辑，使用现有“打开配置目录”处理其文件。
